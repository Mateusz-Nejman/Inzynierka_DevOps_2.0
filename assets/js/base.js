...
function showNotification(state = 0, text = "") {
    ...
    const template = '<div class="notification" id="notification' + notificationId + '"><i class="fas fa-' + icon + ' notificationIcon ' + stateClass + '"></i><div class="notificationText">' + text + '</div></div>';
...
function generateAvatarFromEmail(email) {
    ...
    const emailFormatted = emailPart.replaceAll(".", "[SEP]").replaceAll("_", "[SEP]");
    ...
    if (emailItems.length == 1) {
        ...
        avatarLetters = emailItems[0].charAt(0).toUpperCase() + emailItems[1].charAt(0).toUpperCase();
    }

    return '<div class="taskItemAvatar">' + avatarLetters + '</div>';
}
